...
count_families_site_RLS_sub <- count_families_site_RLS %>%
  filter(province %in% c("Southeast_Polynesia", "Tropical_Northwestern_Atlantic", "Tropical_Southwestern_Pacific", "Western_Coral_Triangle", "Western_Indian_Ocean"))

RLS_fam <- count_families_site_RLS_sub[,c("site35", "family", "prop")]
...
# Total dbRDA
edna.tot.dbrda  <- dbrda(edna_fam ~  (province*n_motus_total), data = met_eDNA, distance = "bray")
RsquareAdj(edna.tot.dbrda)
anova(edna.tot.dbrda)
anova(edna.tot.dbrda, by = "axis",  permutations = 9999)
anova(edna.tot.dbrda, by = "term", permutations = 9999)
...
anova(rls.tot.dbrda)
anova(rls.tot.dbrda, by = "axis",   permutations = 999)
anova(rls.tot.dbrda, by = "term", permutations = 999)

# partial dbRDA for Province
rls.part.dbrda1  <- capscale(RLS_fam ~ province + Condition(n_sp_total), data = met_rls, distance = "bray")
RsquareAdj(rls.part.dbrda1)
anova(rls.part.dbrda1)
anova(rls.part.dbrda1, by = "axis",   permutations = 999)
anova(rls.part.dbrda1,by="margin", permutations = 99)


# partial dbRDA for site richness
rls.part.dbrda2  <- capscale(RLS_fam ~ n_sp_total + Condition(province), data = met_rls, distance = "bray")
RsquareAdj(rls.part.dbrda2)
anova(rls.part.dbrda2)
anova(rls.part.dbrda2, by="margin", permutations = 99)
...
grda_sites <- ggplot(site_scores_region, aes(x= CAP1, y = CAP2)) +
  ...
  theme_bw() +
  theme(axis.line = element_line(colour = "black"),
        legend.position = c(1, 1),             # position in top left corner
